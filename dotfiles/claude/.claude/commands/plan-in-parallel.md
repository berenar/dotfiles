...
4. Launch one Agent per item in parallel, using the Agent tool with `subagent_type: general-purpose`. Send all agent invocations in a single message so they run concurrently.
